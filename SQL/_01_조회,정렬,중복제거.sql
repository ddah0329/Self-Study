...
-- ORDER BY 컬럼명 (ASC 생략가능) : 디폴드 값 


-- 사원의 이름, 성, 봉급 조회 BUT 봉급이 낮은 순서로
...
-- DISTINCT : 중복제거
-----------------------------------------------------------------
-- 회사에 존재하는 직급(JOB_ID)조회
SELECT DISTINCT JOB_ID FROM EMPLOYEES;

-- 여러 개의 컬럼을 조회하는데 DISTINCT 사용
-- -> JOB_ID & HIRE_DATE 동시에 둘 다 동일한 값만 삭제됨
SELECT DISTINCT
	JOB_ID, HIRE_DATE
FROM EMPLOYEES;
...
-- 컬럼에 별칭을 설정했다면 별칭으로 정렬 가능

-----------------------------------------------------------------
-- 사원의 이름, 성, 봉급 조회 *봉급 기준 내림차순 정렬 *별명사용
SELECT 
	FIRST_NAME AS "이름",
	LAST_NAME 성,
	SALARY "봉급",
